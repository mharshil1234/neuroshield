...
const options = [
  'Social Media',
  'Notifications',
  'Too many tabs / information',
  'Noise / Environment',
  'I lose interest quickly',
];

export default function SetupQ1() {
  const navigate = useNavigate();
  const { answers, updateAnswer } = useOnboarding();
  const [selected, setSelected] = useState(answers.distractions || []);
        ...
        <div className="w-full bg-white rounded-[28px] px-8 pt-6 pb-7 shadow-[0_8px_30px_rgb(0,0,0,0.06)] relative z-10">
          ...
          <div className="mb-6">
            ...
            <div className="w-full h-[6px] bg-[#E5EBE6] rounded-full overflow-hidden">
              <div className="h-full bg-[#6B8E73] rounded-full transition-all" style={{ width: '33%' }} />
            </div>
          </div>
          ...
          <div className="text-center mb-5">
            <h1 className="text-[19px] font-bold text-[#2C3E32] leading-snug mb-1">
              What distracts you the most while working?
            </h1>
            <p className="text-[#8FA898] text-[12px]">
              Select the primary factor that impacts your productivity.
            </p>
          </div>
